...
\definecolor{pear}{RGB}{166,133,64} % HKUST's {166,133,63}

% % % % % % % % [Begin] These are configurable % % % % % % %
\FPeval{\lw}{30}        % linewidth
...
\FPeval{\b}{36}         % beta
\FPeval{\c}{18}         % gamma
\FPeval{\ydelta}{1.414} % distance from the top of the pear body to the bottom of the bar
...
\FPeval{\xseed}{1.22}  % You may also change here but the result might not look elegant
...
\draw [pear][line join=round,line cap=round,thick,line width=\lw]  (0,0) arc (90:90-\a:\rone) -- % can also be (-eps,0)
...
\fill [fill=pear,shift={(\xseed,\yseed)},rotate=(\b+\c)/2,scale=0.125]  (0,0) arc (90:90-\a:\rone) -- % can also be (-eps,0)
(\xone,\yone) arc (-90-\a:\b:\rtwo) --
(\xtwo,\ytwo) arc (180+\b:180+\c:\rthree) --
(\xthree,\ythree) arc (\c:180-\c:\rfour) --
(-\xthree,\ythree) arc (-\c:-\b:\rthree) --
...
\draw [pear][line join=round,line cap=round,thick,line width=\lw] (0,\ytopl+\ydelta) to (\ll,\ytop); 
